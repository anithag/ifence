...
let read_process command =
  ...
  let sstring = Bytes.create buffer_size in
  let in_channel = Unix.open_process_in command in
  let chars_read = ref 1 in
  while !chars_read <> 0 do
    chars_read := input in_channel sstring 0 buffer_size;
    Buffer.add_substring buffer (Bytes.to_string sstring) 0 !chars_read
  done;
